import { TProjectShowcase } from '@/types/projects/ProjectShowcase';

export const projects: TProjectShowcase[] = [
  {
    name: 'Hatch Booking System',
    // displayAsset: '/projects/hatch-booking-system',
    href: '/projects/hatch-booking-system',
    description: 'Custom booking system used by McMaster Engineering!',
    tags: ['At Scale'],
  },
  {
    name: 'Island Builder',
    // displayAsset: '/projects/island-builder',
    href: '/projects/island-builder',
    description: 'Create islands with different biomes and connected cities!',
    tags: ['No AI', 'Personal'],
  },
  {
    name: 'MediSafe',
    // displayAsset: '/projects/medisafe',
    href: '/projects/medisafe',
    description: 'Never take conflicting prescriptions again with Medisafe!',
    tags: ['Hackathon Winner', 'Personal'],
  },
  {
    name: 'MonPoke',
    // displayAsset: '/projects/monpoke',
    href: '/projects/monpoke',
    description: 'Catch your favourite MonPokes using python and pygame',
    tags: ['Personal'],
  },
  {
    name: 'Piraten Kapern',
    // displayAsset: '/projects/piraten-kapern',
    href: '/projects/piraten-kapern',
    description: 'A fun implementation of a game with the same name using Java',
    tags: ['Personal'],
  },
  {
    name: 'Infinity Chess',
    // displayAsset: '/projects/infinity-chess',
    href: '/projects/infinity-chess',
    description: 'A Chess variant where pieces can wrap around the walls',
    tags: ['Personal'],
  },
];
