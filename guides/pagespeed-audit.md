...
Speed Index is sitting *directly on* the zero boundary. TBT is still 2.6× past it.
...
| Auto-scroll delay 2000 → 500 ms | **partial** | LCP render delay 5,410 → 2,890 ms, but LCP itself got *worse* (4.3 → 4.6 s). The gate is still there |
...
1. **Three of the fixes gated on visibility, and on this page nothing is ever invisible.** The
   ...
   viewport for the entire scroll. `useIsOnScreen` was the right instinct for the *other*
   routes and a no-op here.
2. **The fixes that helped all made each frame cheaper.** None of them made the frames
   *stop*, and the score is gated on stopping.
...
*scheduling* work. Motion's frame loop commits style writes; the browser then does layout,
...
So the ocean's frame rate *is* the card's paint rate. Two systems designed to be independent
...
Cutting particles made each *canvas* frame cheaper. It did not reduce the *number* of canvas
...
**Visibility was the wrong signal.** The right one is *idleness*: nobody is watching an
...
**Why it got worse rather than better.** LCP always takes the *largest* element, whenever it
...
`0.3 → 0.5` window is measured in *scroll progress*, not time, and an `easeInOut` is slowest
...
`height` keep animating, so the paragraph's box keeps *growing*. A text element that
...
a *separate* 112 KB file — modern browsers skip that one correctly.) `browserslist` governs
how *your* source is transpiled; it doesn't reach inside a prebuilt dependency.
...
*around* it. This is the "general version" the previous round explicitly chose not to do — it
...
const frameloop =
  !onScreen ? 'never'
  : prefersReducedMotion ? 'demand'
  : !userActive ? 'never'      // ← new
  : 'always';
```
...
Tier 1.2 stops the canvas when nobody's looking, which fixes the *measured* cost. This fixes
the *actual* cost, for real visitors who are looking. Options, cheapest first:
...
Everything below documents the previous audit and the work that came out of it. The *findings*
are superseded by the sections above. The *implementation notes* are still current and worth
...
- **The bundle is not the problem.** 226.8 KiB across the flagged chunks is unremarkable, the
  ...
  You have a *per-frame work* problem, and it survived a 130 KiB reduction in initial JS.
